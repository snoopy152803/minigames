...
is hashed into an index into that game's puzzle list. Because it's a pure
function of the date, everyone gets the same puzzle on the same day without
needing a server, and the exact same computation can be re-run later to show
what a past day's answer was. (The "New puzzle"/"New word" button on every
game bypasses this and picks a real random one instead, purely for practice —
those never get marked as "completed" and never show up in the answers
history.) Pips has no daily puzzle or answers page — it's intentionally fully
random every time, with nothing to store or reveal.
