...
## Daily puzzles + the /answers pages

Wordle, Strands, Crossword, Connections, and Spelling Bee each pick a "daily"
puzzle the same way: a day number (days since 2024-01-01, in `shared/daily.js`)
maps to an index into that game's puzzle list via a fixed, deterministic
permutation of `[0, list.length)` — not a plain `day % length` (too orderly:
consecutive days would visibly march through the list) and not a one-off hash
either (hashing-then-modding samples indices *with* replacement, so two
different days can land on the same puzzle far sooner than `list.length` days
apart, purely by chance). A real permutation guarantees every puzzle appears
exactly once per `list.length`-day cycle — nothing repeats sooner than that,
ever. This matters because it's a pure function of the date: everyone gets
the same puzzle on the same day without needing a server, and the exact same
computation can be re-run later to show what a past day's answer was. (The
"New puzzle"/"New word" button on every game bypasses this and picks a real
random one instead, purely for practice — those never get marked as
"completed" and never show up in the answers history.) Pips has no daily
puzzle or answers page — it's intentionally fully random every time, with
nothing to store or reveal.

Because of that guaranteed-no-early-repeat property, the "previous answers"
list for each game caps itself at `min(60, list.length - 1)` days back
(`poolLength` passed into `AnswersPage.render()`) — one day short of a full
cycle. Without that cap, a small puzzle list would eventually let someone
work out today's still-locked answer just by noticing it's identical to an
already-revealed entry from `list.length` days ago. **If you add puzzles to
a list, this stays safe automatically; if a list ever gets very short (under
~15–20 entries), the visible history shrinks accordingly** — that's the
tradeoff, not a bug.

Crossword's puzzle isn't just picked from a list, it's generated by an
algorithm with a lot of internal randomness (word selection *and* layout).
To make "today's crossword" reproducible, `crossword/generator.js` accepts an
optional seeded random function (`shared/seedrandom.js`) instead of always
calling `Math.random()`; the daily puzzle seeds it with the day number, so
regenerating with that same seed later reproduces the identical puzzle.

Each game's script marks that day as "completed" in `localStorage` once you
finish it (win or lose for Wordle/Connections/Crossword's Check; finding the
pangram for Spelling Bee; finding every word for Strands) — see
`window.DailyPuzzle.markCompleted()`. Every game has an **Answers** link in
its header pointing at `<game>/answers/`, which shows:
- **Today's answer** — locked behind a "finish today's puzzle first" message
  until that browser's `localStorage` says you have, then reveals it.
- **Previous answers** — the last 60 days, always visible, since a past
  puzzle isn't a spoiler for anyone.

There's also a `<game>/answers/today/` page for exactly what it says (locked
today's-answer only, with a link out to the full history) if that's the URL
shape you want to link to directly, e.g. `/wordle/answers/today/`.

**Caveat:** the archive is a live computation from the *current* puzzle list,
not a stored history. If you edit a game's list in `admin.html`, every day's
computed answer shifts too — including days already in the past — since
there's no record of what the list looked like on any earlier date. This is
fine as long as the list stays fairly stable; it's not fine if you're
rewriting it constantly and expect the archive to reflect what people
actually played on a given day.
...
- Strands' auto-placed grids use filler letters in unused cells (not a fully
  packed board like the real game). Because filler letters are drawn from the
  puzzle's own word list, a word can occasionally be traceable through more
  than one valid adjacent path — `strands/script.js`'s `checkSelection()`
  accepts any of them (it checks the traced letters against the word list,
  not against one specific stored path), so this doesn't cause incorrect
  rejections; it just means the "official" path in `board.paths[word]` isn't
